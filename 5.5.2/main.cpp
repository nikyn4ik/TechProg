...
int main() {
    ...
    // рандом по установке поездок дл€ студ. карт
    for (int i = 0; i < STUDENT_CARD_COUNT; i++) {
        studentCards[i].setTrips(rand() % 101); // случайное число от 0 до 100
    }

    // выведите кол-во неиспользуемых карт дл€ каждого типа
    std::cout << "Ќеиспользуемые карты метро: " << calculateUnusedCards(metroCards, METRO_CARD_COUNT) << std::endl;
    std::cout << "Ќеиспользуемые студенческие карты: " << calculateUnusedCards(studentCards, STUDENT_CARD_COUNT) << std::endl;
